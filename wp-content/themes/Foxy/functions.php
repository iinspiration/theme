...
function et_foxy_load_scripts_styles(){
	...
	if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'Foxy' ) ) {
		...
		wp_enqueue_style( 'foxy-fonts-open-sans', esc_url( add_query_arg( $query_args, "$protocol://fonts.googleapis.com/css" ) ), array(), null );
	}
	...
	if ( 'off' !== _x( 'on', 'Raleway font: on or off', 'Foxy' ) ) {
		...
		wp_enqueue_style( 'foxy-fonts-raleway', esc_url( add_query_arg( $query_args, "$protocol://fonts.googleapis.com/css" ) ), array(), null );
	}
